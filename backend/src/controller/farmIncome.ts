...
import {
  createFarmIncomeService,
  updateIncomeService,
  patchFarmIncomeService,
  deleteFarmIncomeService,
  getFarmIncomeService,
  getAllFarmIncomeService
} from "../service/farmIncome";

import { body, param, validationResult } from "express-validator";
import { normalize } from "../utils/normalize";
import { DataType } from "../types/dataType";

export const farmIncomeRouter = Router();

farmIncomeRouter.post(
  "/create",
  ...
  async (req: Request, res: any) => {
    ...
    try {
      const income = await createFarmIncomeService(req.body);
      res.send(
        normalize(
          "Farm income created successfully",
          "OK",
          DataType.object,
          income,
        ),
...
farmIncomeRouter.put(
  ...
  async (req: Request, res: any) => {
    ...
    try {
      ...
      res.send(
        normalize(
          "Farm Income updated successfully",
          "OK",
          DataType.object,
          income,
        ),
...
farmIncomeRouter.patch(
  ...
  async (req: Request, res: any) => {
    ...
    const id = BigInt(req.params.id);
    try {
      const inventory = await patchFarmIncomeService(id, req.body);
      res.send(inventory);
    } catch (error) {
...
farmIncomeRouter.delete(
  ...
  async (req: Request, res: any) => {
    ...
    const id = BigInt(req.params.id);
    try {
      await deleteFarmIncomeService(id);
      res
        .status(200)
        .json(
          normalize(
            "Farm Income deleted successfully",
            "OK",
            DataType.null,
            null,
          ),
...
farmIncomeRouter.get(
  ...
  async (req: Request, res: any) => {
    ...
    const id = BigInt(req.params.id);
    try {
      const income = await getFarmIncomeService(id);
      if (income) {
        res.send(
          normalize(
            "Farm Income Detail found successfully",
            "OK",
            DataType.object,
            income
          ),
        ...
        res
          .status(404)
          .json(normalize("Farm Income not found", "ERROR", DataType.null, null));
      }
    } catch (error) {
...
farmIncomeRouter.get("/", async (_req: Request, res: Response) => {
  try {
    ...
    const income = await getAllFarmIncomeService({
      incomeTypeId,
      page,
      limit,
      search,
    ...
    res.send(
      normalize("Farm Income found successfully.", "OK", DataType.array, {
        income
      }),
    );
  } catch (error) {
